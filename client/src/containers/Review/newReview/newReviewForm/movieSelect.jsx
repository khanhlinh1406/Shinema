    ...
    const getMovies = async () => {
        try {
            if (keyword != null && keyword != '') {
                ...
                dispatch(movieSlice.actions.updateListFromReviewSelect(response.results.filter(
...
const ListSuggestions = () => {
    ...
    return (
        <Stack sx={{ maxHeight: 300, width: 350, overflowY: 'scroll', backgroundColor: 'rgb(9, 24, 48)' }}>
            {
                listFromReviewSelect && listFromReviewSelect.map((item, index) => (
...
const SuggestionItem = ({ item }) => {
    ...
    const onClickItem = (item) => {
        dispatch(movieSlice.actions.updateSelectForReview(item))
    }
    ...
    return (
        <Stack direction='row' sx={{ marginY: 0.5, width: '100%' }}>
            ...
            <CustomItemSelectButton onClick={() => onClickItem(item)} variant="text">{item.title}</CustomItemSelectButton>
        </Stack>
...
const CustomItemSelectButton = styled(Button)(({ theme }) => ({
    color: '#fff',
