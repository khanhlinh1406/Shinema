...
const Header = ({ userId, reviewItem }) => {
    ...
    useEffect(() => {
        const getUser = () => {
            AccountApi.getById(userId).then(res => {
                if (res.status == 200) {
                    setUser(res.data)
                }
...
const ChangeStatusHandle = () => {
    let status = useSelector(state => state.review.current.status)
    let reviewId = useSelector(state => state.review.current._id)
    let censorId = useSelector(state => state.users.instance._id)
    ...
    const handleAgree = () => {
        ...
        ReviewApi.update(reviewObj).then(res => {
            if (res.status = 200) {
    ...
    return (
        <Stack>
            {status == 'Inspected' &&
                <InspectedButton variant="contained" endIcon={<PublicRoundedIcon />} onClick={() => handleClickOpen('Inspected')}>
                    Inspected
                </InspectedButton>
            }

            {status == 'Uninspected' &&
                <UninspectedButton variant="contained" endIcon={<PublicOffRoundedIcon />} onClick={() => handleClickOpen('Uninspected')}>
                    Uninspected
                </UninspectedButton>
            }

            {status == 'Inspecting' &&
                <InspectingButton variant="contained" endIcon={<DonutLargeRoundedIcon />} onClick={() => handleClickOpen('Inspecting')}>
                    Inspecting
                </InspectingButton>
            }
...
const Comment = ({ data }) => {
    ...
    return (
        <Stack sx={{ backgroundColor: 'rgb(23, 32, 48)', marginTop: 5, padding: 2, overflowY: 'scroll', maxHeight: 400 }}>
            <SendBox />
            {
                data && data.map((item, index) => (
                    <CmtItem item={item} key={index} />
                ))
            }
        </Stack>
    )
...
const SendBox = () => {
    ...
    const sendMessageHandle = (text) => {
        ...
        ReviewApi.insertCmt(cmtObj).then(res => {
            if (res.status == 200) {
        ...
        <div>
            {
                ...
                <Stack direction='row' sx={{ marginY: 1.5 }}>
                    <Avatar sx={{ bgcolor: backgroundColorAvatar }}>{user.name.charAt(0)}</Avatar>
                    <Stack >
                        <Stack sx={{ backgroundColor: "rgb(18, 49, 100)", padding: 1.5, marginLeft: 2, borderRadius: 2 }}>
                            <Typography sx={{ fontWeight: 'bold' }}>{user.name}</Typography>
